...
def make_server_contract(contract_stream, server_nym):
...
def setup(contract_stream, total_servers=1):
    ...
    server_contract, cached_key, decoded_signed_contract \
        = make_server_contract(contract_stream, server_nym)
    ...
    for _ in range(total_servers - 1):
        opentxs.OTAPI_Wrap_AddServerContract(
            make_server_contract(contract_stream, nym.Nym().create())[2])
